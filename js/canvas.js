function runCanvas() {
  ...
  function circle201() {
    if (circle2.originY + 100 >= canvas.height && circle2.originX >= 920) {
      circle2.speedX += 8;
      circle2.speedY = -circle2.speedY;
      setTimeout(() => { circle202(); }, 500);
    }
  ...
  function draw() {
    ...
    ctx.arc(circle3.originX, circle3.originY, 70, 0, 2 * Math.PI);
    ctx.fill();
    ctx.beginPath();
    ctx.fillStyle = circle4.color;
    ctx.arc(circle4.originX, circle4.originY, 70, 0, 2 * Math.PI);
    ctx.fill();
    circle101(circle1);
    circle201(circle2);
